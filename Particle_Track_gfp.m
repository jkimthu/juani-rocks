function ParticleOut=Particle_Track_gfp(particles, trackmode, Rmax, matchmethod)
...
for n = 1:N_frames
    ...
    particleMat=[particles(n).X,particles(n).Y,particles(n).A,particles(n).MajAx,particles(n).MinAx,particles(n).gfp,ones(size(particles(n).X)).*particles(n).Frame,particles(n).Ecc,particles(n).Angle];
    % 1 = X
    % 2 = Y
    % 3 = Area
    % 4 = Major axis
    ...
    ParticleOut=cellfun(@(A) struct('X',A(:,1),'Y',A(:,2),'Area',A(:,3),'MajAx',A(:,4),'MinAx',A(:,5),'GFP',A(:,6),'Frame',A(:,7),'Ecc',A(:,8),'Angle',A(:,9)),ParticleTracks); 
end