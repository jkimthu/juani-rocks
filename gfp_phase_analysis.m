...
% Goal: builds a dataset by particle tracking and measuring parameters of
%       interest from cells imaged in phase and GFP
%
%       input data is a timelapse with 2 channels: phase and 2FP
...
% last update: Jen, 2022 Jan 19
% commit: edited from glycogen_analysis.m for juani's GFP experiments
...
%              fluorescent label 
 %      
%                    1) compute the GFP fluorescence intensity of each cell
%                    2) choose a threshold for determining if a cell is on or off in GFP
...
for xy = 1:xy_final
    ...
    for i = 1:length(selected_tpt)
        ...
        g_name = strcat(imageName,tptName,xyName,'c2.tif');

        
        % load image into matlab with function "imread"
        ph_image = imread(ph_name);
        g_image = imread(g_name);
        clear ph_name g_name 
        ...
        g_mask_overlay = bw_final .* double(y_image); % convert your uint16 image to double class
        ...
        for c = 1:cc.NumObjects;
            ...
            g_fluo_int_cells(c) = mean(g_mask_overlay(pixel_id_of_cell)); % compute the mean intensity of pixels in the cell
        ...
        for c = 1:cc.NumObjects;
            ...
            % gfp
            stats(c).gfp = g_fluo_int_cells(c);
           
        end
        clear ph_image g_image g_mask_overlay
